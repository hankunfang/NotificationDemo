...
/**
 * A Switch is a two-state toggle switch widget that can select between two options. The user may drag the "thumb" back
 * and forth to choose the selected option, or simply tap to toggle as if it were a checkbox. The
 * {@link #setText(CharSequence) text} property controls the text displayed in the label for the switch, whereas the
 * {@link #setTextOff(CharSequence) off} and {@link #setTextOn(CharSequence) on} text controls the text on the thumb.
 * Similarly, the {@link #setTextAppearance(android.content.Context, int) textAppearance} and the related setTypeface()
...
public class ToggleSwitch extends CompoundButton {
    ...
    /**
     * Construct a new Switch with a default style determined by the given theme attribute, overriding specific style
     * attributes as requested.
     * 
     * @param context The Context that will determine this widget's theming.
     * @param attrs Specification of attributes that should deviate from the default styling.
     * @param defStyle An attribute ID within the active theme containing a reference to the default style for this
     * widget. e.g. android.R.attr.switchStyle.
     */
    ...
    /**
     * Sets the switch text color, size, style, hint color, and highlight color from the specified TextAppearance
     * resource.
     */
    public void setSwitchTextAppearance(Context context, int resid) {
        ...
        colors = appearance.getColorStateList(R.styleable.TextAppearance_showTextColor);
        if (colors != null) {
            mTextColors = colors;
        } else {
        ...
        ts = appearance.getDimensionPixelSize(R.styleable.TextAppearance_showTextSize, 0);
        if (ts != 0) {
    ...
    /**
     * Sets the typeface and style in which the text should be displayed on the switch, and turns on the fake bold and
     * italic bits in the Paint if the Typeface that you provided does not have all the bits in the style that you
     * specified.
     */
    ...
    /**
     * Sets the typeface in which the text should be displayed on the switch. Note that not all Typeface families
     * actually have bold and italic variants, so you may need to use
